    ...
    <div x-data="{ tab: 'tree' }" class="space-y-4">
        ...
        <div x-show="tab === 'tree'" x-data="genealogyActions()">
            <div class="flex items-center justify-between mb-2 gap-2">
                ...
                {{-- Share link --}}
                <div x-show="shareUrl" class="flex items-center gap-1.5 text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 min-w-0 flex-1">
                    <svg class="w-3.5 h-3.5 shrink-0 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
                    <span x-text="shareUrl" class="truncate font-mono" style="max-width:240px;"></span>
                    <button @click="copyShareUrl()" class="shrink-0 text-primary-600 hover:underline font-medium" x-text="copied ? 'Đã sao chép!' : 'Sao chép'"></button>
                </div>
                <div x-show="!shareUrl" class="flex-1"></div>

            <div class="flex items-center gap-2">
                {{-- Import GEDCOM --}}
                <a href="{{ route('genealogy.import-gedcom') }}"
                   ...
                   title="Import từ Ancestry, FamilySearch, MacFamilyTree...">
                ...
                {{-- Xuất GEDCOM --}}
                <a href="{{ route('genealogy.export-gedcom') }}"
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
                   title="Tương thích Ancestry, FamilySearch, MacFamilyTree">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- <button @click="toggleShare()"
                        :class="shareUrl ? 'bg-green-50 border-green-200 text-green-700 hover:bg-green-100' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'"
                    ...
                    <span x-text="shareUrl ? 'Đang chia sẻ' : 'Chia sẻ'"></span>
                </button> --}}
    ...
    function genealogyActions() {
        return {
            ...
            async toggleShare() {
                ...
                if (this.shareUrl) {
                    // Đang chia sẻ → tắt đi
                    await fetch('{{ route('share-tree.disable') }}', {
                        method: 'POST',
                    ...
                    // Bật chia sẻ → lấy URL
                    const res  = await fetch('{{ route('share-tree.enable') }}', {
                        method: 'POST',
                        headers: { 'X-CSRF-TOKEN': csrf },
                    });
